(defpackage :fwoar.cl-git.git-objects.packed
  (:use :cl )
  (:export ))
(in-package :fwoar.cl-git.git-objects.packed)

(fiveam:def-suite :fwoar.cl-git.git-objects.packed
  :description "testing branch resolution"
  :in :fwoar.cl-git.git-objects)
(fiveam:in-suite :fwoar.cl-git.git-objects.packed)

(defparameter *fake-repo* :fwoar.cl-git.git-objects.packed)
...
(fiveam:def-test pack-files-commit ()
  (let* ((hash "7d7b56a6a64e090041f55293511f48aba6699f1a")
         (ref (fwoar.cl-git::packed-ref *fake-repo* hash))
